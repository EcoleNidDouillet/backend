-- École Nid Douillet - CMS Content Table Migration (Fixed)
-- Content Management System for public website

-- Create CMS content table without foreign key constraint initially
CREATE TABLE IF NOT EXISTS cms_content (
    ...
    created_by INTEGER -- Will add foreign key constraint later if directors table exists
);

-- Add foreign key constraint only if directors table exists
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'directors') THEN
        -- Check if foreign key constraint doesn't already exist
        IF NOT EXISTS (
            SELECT 1 FROM information_schema.table_constraints 
            WHERE constraint_name = 'cms_content_created_by_fkey'
        ) THEN
            ALTER TABLE cms_content 
            ADD CONSTRAINT cms_content_created_by_fkey 
            FOREIGN KEY (created_by) REFERENCES directors(id);
        END IF;
    END IF;
END $$;
...
-- Use created_by = 1 if directors table exists, otherwise NULL
...
(
    ...
    true,
    1,
    CASE WHEN EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'directors') THEN 1 ELSE NULL END
),
