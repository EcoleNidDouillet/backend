-- École Nid Douillet - CMS Content Table Migration (Clean)
-- Content Management System for public website

-- Drop existing cms_content table if it exists (for clean setup)
...
CREATE UNIQUE INDEX idx_cms_content_slug_language 
ON cms_content(slug, language);

-- Create indexes for better performance
CREATE INDEX idx_cms_content_published 
ON cms_content(is_published, published_at DESC) WHERE is_published = true;

CREATE INDEX idx_cms_content_type 
ON cms_content(content_type, language, is_published);

CREATE INDEX idx_cms_content_display_order 
ON cms_content(display_order, published_at DESC);

CREATE INDEX idx_cms_content_tags 
ON cms_content USING GIN(tags);

CREATE INDEX idx_cms_content_language 
ON cms_content(language, is_published);

CREATE INDEX idx_cms_content_created_by 
ON cms_content(created_by);
